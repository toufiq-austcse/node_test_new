...
export class MenuItemsService {
    ...
    */
    async getMenuItems() {
        let result = await this.menuItemRepository.query(
            `WITH RECURSIVE menu_item_path (id, name, url,parentId, createdAt) AS(
  SELECT id, name, url,parentId, createdAt as path
    FROM menu_item
    WHERE parent_id IS NULL
  UNION ALL
  SELECT c.id, c.name,c.parentId
    FROM menu_item_path AS cp JOIN menu_item AS c
      ON cp.parentId = c.id
)
